...
# INITIAL = np.array([[1, 1], [1, 2], [1, 3], [2, 1], [2, 2], [2, 3], [3, 1], [3, 2], [3, 3]])
KNOW_GOAL = False
...
class Maze(tk.Tk, object):
    def __init__(self):
        ...
        self.action_space = ['u', 'd', 'l', 'r', 'u2', 'd2', 'l2', 'r2']
    ...
    def step(self, action):
        ...
        if action == 0:  # up
            ...
            updated_cor = self.agent + np.array([0, -2])
            if updated_cor[1] >= 1 and (updated_cor.tolist()
                                        not in self.block.tolist()):
                self.agent = updated_cor
            else:
            ...
            if updated_cor[1] <= self.maze_h and (updated_cor.tolist()
                ...
                hit = True
        elif action == 6:  # right2
            updated_cor = self.agent + np.array([2, 0])
            if updated_cor[0] <= self.maze_w and (updated_cor.tolist()
                                        not in self.block.tolist()):
                self.agent = updated_cor
            else:
