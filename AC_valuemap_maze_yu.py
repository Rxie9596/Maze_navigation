...
PUNISHMENT = True
SAVE = True
save_directory = '/Users/yuxie/Lab/Maze_navigation/Data/env' + env_name + '_' + exp + '_valuemap' + '.mat'
step_name = 'steps' + env_name + '_' + exp + '_valuemap'
reward_name = 'reward' + env_name + '_' + exp + '_valuemap'
smooth_r_name = reward_name + "_smooth"
valuemap_name = 'valuemap' + env_name + '_' + exp
...
valuemap = np.zeros((env.maze_h,env.maze_w))
for witer in range(env.maze_w):
    for hiter in range(env.maze_h):
        if [witer+1, hiter+1] not in env.block.tolist():
            state = np.zeros(env.maze_w + env.maze_h)
            state[witer] = 1.0
            state[env.maze_w + hiter] = 1.0
            state = np.concatenate((state, env.goalstate))
            valuemap[hiter, witer] = critic.value(state)
        else:
            valuemap[hiter, witer] = -1
